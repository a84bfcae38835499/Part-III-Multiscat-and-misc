...
def find_mean_stdv_with_stdvin(values, sigmas):
    ...
    values = np.array(values)
    #print("sigmas/values = ")
    #print(sorted(sigmas/values))
    variences = np.array(sigmas)**2
    #print("vars = ")
    #print(variences)
    #print("type of input = " + str(type(values)))
    #n = len(values)
    ...
    #print(f"Mean**2, msq = {mean**2}, {meanSq}")
    stdv = np.sqrt(1/invMeanSq)
    return np.array(mean), np.array(stdv)
...
def tiny_S(I_I0, I_I0unc):
    Seeesz = cellArea * ((1-np.sqrt(I_I0))/Theta)
    Sunc = I_I0unc*(cellArea/(Theta*np.sqrt(I_I0)))
    return Seeesz, Sunc
...
fileprefixes = ['_5x5_01D']
fileprefixes = ['_5x5_0' + str(x) + 'D' for x in range(1,7)]

fileprefixes = ['gv5x5_0' + str(x) + 'D' for x in range(1,10)]
fileprefixes += ['gv5x5_' + str(5*x) + 'D' for x in range(2,6)]

fileprefixes = ['3x3MoS2']
...
n1n2OfInterest = [[1,0],
                  ...
                  [1,-2],
                  [-1,2]]
n1n2Colours = np.array([[1, 0.82, 0.149],
                  [1, 1, 1],
                  [0.067, 0.769, 0.451],
                  [0.149, 0.792, 0.7],
                  [0.296, 0.369, 1],
...
"""
Ninterest = sum(1 for _ in n1n2OfInterest)
Nfileprefix = len(fileprefixes)

results = np.zeros((Ninterest,len(fileprefixes)))
resultsunc = np.zeros((Ninterest,len(fileprefixes)))


for index_f in range(Nfileprefix):
    fileprefix = fileprefixes[index_f]
    if(scatcondprefix == ''):
        scatcondprefix = fileprefix
    ...
    for index_s in range(Nscat):
        ...
        entropiesDisposable = np.zeros((Nensemble))
        kAvgAbsDisposable = np.zeros((Nensemble))
        IsOfIDisposable = np.zeros((Nensemble,Ninterest))
        RatiosOfIDisposable = np.zeros((Nensemble,Ninterest))
        RatiosOfIUncDisposable = np.zeros((Nensemble,Ninterest))
        RatiosDisposable = []
        RatiosUncDisposable = []
        TinySDisposable = np.zeros((Nensemble,nOccChPrisArr[index_s]))
        #print("1/N = " + "{:.7f}".format(1/float(nOccChArr[index_s])))
        for index_n in range(Nensemble):
            nDiffuse = 0
            nDiffract = 0
        ...
        for index_n in range(Nensemble):
                        ...
                        for ch in range(nOccChPrisArr[index_s]):
                            ...
                            if(N1 == n1 and N2 == n2):
                                ...
                                if(diffuseCompensationMode == 1):
                                    ...
                                    I-=meanDiffuseIntensity
                                if(I < Ipris):
                                    RatiosDisposable.append(I/Ipris)
                                    RatiosUncDisposable.append(mDIUnc/Ipris)
                                #else:
                                #    print(f"Tactically ignoring {n1,n2},\nI = {I}\nI_0={Ipris}\nMDI={meanDiffuseIntensity}")
                                TinySDisposable[index_n,ch],null = tiny_S(I/Ipris,0.1337)
            ...
            kAvg += kA / float(Nensemble)
            entropiesDisposable[index_n] = calculate_entropy(iI[index_n])
            kAvgAbsDisposable[index_n] = np.sqrt(kA[0]**2+kA[1]**2)
        #print("kAbsDisposable = ")
        #print(kAbsDisposable)
        #print("IsOfIDisposalbe = ")
        #print(IsOfIDisposable)
        ...
        if(pristineprefix != ""):
            ...
            #print("RatiosEnsUnc = ")
            #print(RatiosEnsUnc)
            TinySEns = np.reshape(TinySDisposable,(np.size(TinySDisposable),))
            #RatiosAvgArr[index_s], RatiosUncArr[index_s] = find_mean_stdv_with_stdvin(RatiosEns,RatiosEnsUnc)
            RatiosAvgArr[index_s], RatiosUncArr[index_s] = find_mean_stdv(RatiosEns)
            TinySAvgArr[index_s], TinySUncArr[index_s] = find_mean_stdv(TinySEns)
            for index_i in range(Ninterest):
                Ratio1n2ens = RatiosOfIDisposable[:,index_i]
                #print("Ratio1n2ens = ")
                #print(Ratio1n2ens)
        ...
        coordYArr[index_s] = cY[0]
        entropiesOut[index_s], entropiesOutUnc[index_s] = find_mean_stdv(entropiesDisposable)
        kAbsAvgArr[index_s], kAbsAvgUncArr[index_s] = find_mean_stdv(kAvgAbsDisposable)
        #print("kAbsAvg = ")
        #print(kAbsAvgArr[index_s])
        #print("Unc = ")
        #print(kAbsAvgUncArr[index_s])
    ...
    for index_s in range(Nscat):
        ...
        print(meanX, meanY)
        print("Mean = ")
        print(np.sqrt(meanX**2+meanY**2)*Babs/Nsuper)
    ...
    # print("Diffuse intensity proportion  : " + str(normDiffI))
        ...
        #print("I/I_0 (tactical) = " + "{:.7f}".format(RatiosAvgArr[index_s]) + " ± " + "{:.7f}".format(RatiosUncArr[index_s]))
        csstr = "$\S_{tiny} = $" + "{:.4f}".format(crossSectionWhole) + " ± " + "{:.4f}".format(crossSectionWholeUnc) + "Å$^2$"
        #print("Total S tiny = " + "{:.7f}".format(crossSectionWhole) + " ± " + "{:.7f}".format(crossSectionWholeUnc) + " Å^2")
        #print(simgastr)
        if(Ninterest > 0):
            for index_i in range(Ninterest):
                ...
                #print( prefix.ljust(12) + "{:.7f}".format(IsOfIAvgArr[index_s][index_i]) + " ± " + "{:.7f}".format(IsOfIUncArr[index_s][index_i]))
                if(pristineprefix != ""):
                    print( "I/I0"+str(n1n2) +" = " + "{:.7f}".format(RatiosOfIAvgArr[index_s][index_i]) + " ± " + "{:.7f}".format(RatiosOfIUncArr[index_s][index_i]))
                    results[index_i,index_f] = (RatiosOfIAvgArr[index_s][index_i])
                    resultsunc[index_i,index_f] = (RatiosOfIUncArr[index_s][index_i])
                    #print("Σ_" + str(n1n2OfInterest[index_i]) +" = " + "{:.7f}".format(SigmasOfIAvgArr[index_s][index_i]) + " ± " + "{:.7f}".format(SigmasOfIUncArr[index_s][index_i]) + "Å^2")
                #print(": : : : : : : : : : : : : : : : ")
        ...
        if(plotFigure):
            ...
            for b in bools:
                ...
                heliumk = heliumDir * np.sqrt(2*m*e*E/1000)/hbar
                print("heliumk / A^-1 = ")
                print(1e-10*np.sqrt(2*m*e*E/1000)/hbar)
                heliumk_n = heliumk/(Babs*1e10)
                if(not vanity):
                    if(showB):
                        plt.arrow(0,0,b1[0]*Nsuper,b1[1]*Nsuper,width=0.05,color=b1col,zorder=7,linewidth=0.5,length_includes_head=True)
                ...
                if(not vanity):
                    ...
                    for index_i in range(Ninterest):
                        ...
                        n2 = n1n2[1]
                        ax2.scatter(Nsuper*(b1[0]*float(n1)+b2[0]*float(n2)),Nsuper*(b1[1]*float(n1)+b2[1]*float(n2)),
                                    marker=(6, 0, 90),color=n1n2Colours[index_i], zorder=6,facecolors='none',s=hexsize,linewidth=1)
                        if(pristineprefix != "" and showIndividualRatios):
                            sstr = "I/I$_0$=\n" + "{:.2f}".format(RatiosOfIAvgArr[index_s][index_i]) 
                            if(Nensemble > 1 and extractMicrostate == 0):
                                sstr+="$\pm$\n"+"{:.2f}".format(RatiosOfIUncArr[index_s][index_i])
                            ax2.annotate(sstr,(Nsuper*(b1[0]*float(n1)+b2[0]*float(n2))+0.75,Nsuper*(b1[1]*float(n1)+b2[1]*float(n2))),color=n1n2Colours[index_i],
                                        fontsize = sigmaFontSize,zorder=12,ha='left',va='bottom')
                ...
                heliumk = heliumDir * np.sqrt(2*m*e*E/1000)/hbar
                print("heliumk / A^2 = ")
                print(1e10*np.sqrt(2*m*e*E/1000)/hbar)
                heliumk_n = heliumk/(Babs*1e10)
                if(not vanity):
                    #print("heliumk_n =")
                    #print(heliumk_n)
                ...
                if(not vanity):
                    if(writeCaption):
                        plt.figtext(0.5, -0.035, defectstr+", "+entropytxt, wrap=True, horizontalalignment='center', fontsize=captionFontSize,transform=ax2.transAxes)
                        plt.figtext(0.5, -0.07, intenstr + ", "+kstr_txt, wrap=True, horizontalalignment='center', fontsize=captionFontSize,transform=ax2.transAxes)
                else:
                    filenametxt = "vanity"
                    plt.tight_layout(pad=0)
                    fig.patch.set_facecolor('k')
