...
Thetas = np.array([x/25 for x in range(0,10)]+[x/25 for x in range(10,30,5)])
...
Adatom_H_diff = np.array([0.482082, 0.520447,0.548192,0.572525,0.595165,0.614375,0.639014,0.654021,0.673540,0.688574,0.702831,0.755526,0.849853,0.890095
])
Adatom_H_diff_Unc = np.array([0, 0.004248,0.003308,0.004460,0.005410,0.006033,0.006615,0.006312,0.005530,0.005966,0.003914,0.007344,0.211946,0.265483
])
...
#])
Adatom_K = np.array([
1.8196899,1.8319695,1.8632271,1.8917125,1.8799515,1.9361424,1.9477458,1.9530034,1.9804537,2.0129283,2.0777183,2.1250425,2.5401360,2.6475588
])
Adatom_K_Unc = np.array([
0.,0.0262528,0.0497557,0.0474716,0.0845696,0.0878799,0.0696341,0.0722962,0.0990899,0.0931299,0.1327683,0.1102943,1.0166162,1.1345351
])


Vacancy_H_diff = np.array([0.482082,0.518472,0.546852,0.571712,0.594904,0.621270,0.636488,0.653600,0.674068,0.687810,0.701956,0.765958,0.800688,0.834114
])
Vacancy_H_diff_Unc = np.array([0,0.004146,0.002800,0.007577,0.004823,0.004684,0.005989,0.005131,0.004122,0.011551,0.005445,0.007510,0.008652,0.006585
])
Adatom_K = 5.596233671922362-Adatom_K
...
#])
Vacancy_K = np.array([
1.8196899,1.8337429,1.8547909,1.8771613,1.9104914,1.9690134,1.9689019,2.0050719,1.9985184,2.0403300,2.0185044,2.1599095,2.3094884,2.3944890
])
Vacancy_K_Unc = np.array([
0.,0.0188508,0.0375486,0.0332651,0.0922589,0.0565959,0.0807228,0.0896489,0.0740899,0.0852711,0.0709918,0.1094006,0.1403835,0.1579918
])
Vacancy_K = 5.596233671922362-Vacancy_K


fig = plt.figure()
ax1 = fig.add_subplot(111)
plt.title("Entropy and |$\\langle K_{out} \\rangle$| - Adatoms")
ax1.set_ylabel("H$_{diff}$")
ax2 = ax1.twinx()
ax2.set_ylabel("|$K$| / $Å^{-1}$")
ax1.set_xlabel("$\Theta$")
ax1.set_xlim(min(Thetas),max(Thetas)+0.05)
ax1.set_ylim(min(Adatom_H_diff),0.9)
ax2.set_ylim(min(Adatom_K)-+max(Adatom_K_Unc),4)
...
ax2.errorbar(x=Thetas,y=Adatom_K,yerr=Adatom_K_Unc,label="Adatom |$K$|",
                 color=[1,.5,0.],linestyle=(0,(4,10)),marker='o',markersize=5,capsize=2.)
...
plt.title("Entropy and |$\\langle K_{out} \\rangle$| - Vacancies")
ax1.set_ylabel("H$_{diff}$")
ax2 = ax1.twinx()
ax2.set_ylabel("|$K$| / $Å^{-1}$")
ax1.set_xlabel("$\Theta$")
ax1.set_xlim(min(Thetas),max(Thetas)+0.05)
ax1.set_ylim(min(Vacancy_H_diff),0.9)
ax2.set_ylim(min(Vacancy_K)-max(Vacancy_K_Unc),3.8)
...
ax2.errorbar(x=Thetas,y=Vacancy_K,yerr=Vacancy_K_Unc,label="|$K$|",
                 color=[0.,0.5,0.5],linestyle=(0,(4,10)),marker='o',markersize=5,capsize=2.)
